...
def load_env_and_fetch_data(source_id: str, parameter: int = 1):
    ...
    # Send parameter along with source_id in the API URL
    API_URL = f"{BASE_API_URL.rstrip('/')}/{source_id}/{parameter}"
    headers = {
...
def run_assignment(source_id: str, parameter: int = 1):
    """
    ...
    try:
        print(f"🚀 Starting simplified enhanced assignment for source_id: {source_id}, parameter: {parameter}")

        # Load and validate data
        data = load_env_and_fetch_data(source_id, parameter)
