...
def display_detailed_analytics(result):
    ...
    for i, route in enumerate(routes):
        ...
        # Get driver source info by checking if driver is in unassigned_drivers list
        driver_source = "driversAssigned"  # Default assumption
        shift_type_display = "ST:N/A"
        
        # Check if this driver was originally from driversUnassigned
        driver_found_in_unassigned = False
        for unassigned_driver in unassigned_drivers:
            if str(unassigned_driver.get("driver_id", "")) == str(route["driver_id"]):
                driver_source = "driversUnassigned"
                shift_type_display = f"ST:{unassigned_driver.get('shift_type_id', 'N/A')}"
                driver_found_in_unassigned = True
                break
        
        # If not found in unassigned, it must be from driversAssigned
        if not driver_found_in_unassigned:
            driver_source = "driversAssigned"
            # We don't have shift_type info for assigned drivers in the current structure
    ...
    for route in routes:
        ...
        # Check unassigned drivers first
        for d in unassigned_drivers:
            if d.get("driver_id") == driver_id or str(d.get("id", "")) == driver_id:
                shift_type_id = d.get('shift_type_id', 2)
                if shift_type_id in [1, 3]:
                    priority_stats["P1"] += 1
                    priority_name = "Priority 1 (driversUnassigned ST:1,3)"
                else:
                    priority_stats["P2"] += 1
                    priority_name = "Priority 2 (driversUnassigned ST:2)"
        ...
        if not driver_found:
            # This driver was likely from driversAssigned
            priority_stats["P3_P4"] += 1
            detailed_driver_info.append({
                "driver_id": driver_id,
                "source": "driversAssigned",
                "shift_type_id": "N/A",
                "priority": "Priority 3/4 (driversAssigned)",
                "capacity": route["vehicle_type"],
                "users_assigned": len(route["assigned_users"]),
                "utilization": len(route["assigned_users"]) / route["vehicle_type"] * 100
            })
    ...
    for i, driver_info in enumerate(detailed_driver_info, 1):
        ...
        print(f"   {i:2d}. Driver {driver_info['driver_id']} | {driver_info['source']:16} | "
              f"ST:{driver_info['shift_type_id']:2} | Cap:{driver_info['capacity']} | "
    ...
    if p1_percentage > 60:
        print("   ✅ EXCELLENT: System prioritized driversUnassigned effectively")
