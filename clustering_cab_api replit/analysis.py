...
def test_api_connection_debug(source_id: str):
    ...
    API_URL = f"{BASE_API_URL.rstrip('/')}/{source_id}/1/Morning%20shift"
...
def comprehensive_analysis(source_id: str, parameter: int = 0, string_param: str = "", output_file: str = "analysis_report.json"):
    """
    ...
    try:
        ...
        try:
            print(f"🌐 Fetching data for source_id: {source_id}, parameter: {parameter}, string_param: {string_param}")
            # Import the function from assignment module
            from assignment import load_env_and_fetch_data
            raw_data = load_env_and_fetch_data(source_id, parameter, string_param)
        ...
        print("📊 Analyzing data structure and quality...")
        users_data = raw_data.get("users", [])

        # Handle nested drivers structure
        if "drivers" in raw_data:
            drivers_data = raw_data["drivers"]
            drivers_unassigned_data = drivers_data.get("driversUnassigned", [])
            drivers_assigned_data = drivers_data.get("driversAssigned", [])
        else:
            drivers_unassigned_data = raw_data.get("driversUnassigned", [])
            drivers_assigned_data = raw_data.get("driversAssigned", [])

        # Combine all drivers for analysis
        all_drivers = (drivers_unassigned_data or []).copy()
        if drivers_assigned_data:
            all_drivers.extend(drivers_assigned_data)

        print(f"🔍 Raw data - Users: {len(users_data)}, driversUnassigned: {len(drivers_unassigned_data)}, driversAssigned: {len(drivers_assigned_data)}")
        ...
        analysis_report["raw_data_analysis"] = {
            ...
            "total_drivers_unassigned": len(drivers_unassigned_data),
            "total_drivers_assigned": len(drivers_assigned_data),
            ...
            "drivers_analysis": analyze_drivers_data(drivers_unassigned_data, drivers_assigned_data),
            "capacity_vs_demand": analyze_capacity_demand(users_data, drivers_unassigned_data, drivers_assigned_data)
        }

        # 2. API RESPONSE STRUCTURE
        all_drivers = drivers_unassigned_data + drivers_assigned_data
        analysis_report["api_response"] = {
            "response_structure": {
                "has_status": "status" in raw_data,
                "has_users": "users" in raw_data,
                "has_drivers_unassigned": "driversUnassigned" in raw_data,
                "has_drivers_assigned": "driversAssigned" in raw_data,
                "additional_fields": [k for k in raw_data.keys() if k not in ["users", "driversUnassigned", "driversAssigned", "status"]]
            },
            "data_quality": assess_data_quality(users_data, all_drivers)
        }
        ...
        if assignment_result["status"] == "true":
            ...
            analysis_report["optimization_insights"] = optimization_insights(routes, users_data, drivers_unassigned_data, drivers_assigned_data)
...
def analyze_drivers_data(drivers_unassigned_data, drivers_assigned_data=None):
    """Analyze drivers data structure and statistics with priority system"""
    if not drivers_unassigned_data and not drivers_assigned_data:
        return {"error": "No drivers data available"}

    # Combine driversUnassigned and driversAssigned
    all_drivers = (drivers_unassigned_data or []).copy()
    if drivers_assigned_data:
        all_drivers.extend(drivers_assigned_data)
...
def analyze_capacity_demand(users_data, drivers_unassigned_data, drivers_assigned_data=None):
    """Analyze capacity vs demand relationship including assigned drivers"""
    total_users = len(users_data)
    total_capacity = sum(int(d.get('capacity', 0)) for d in (drivers_unassigned_data or []))

    if drivers_assigned_data:
        total_capacity += sum(int(d.get('capacity', 0)) for d in drivers_assigned_data)

    # Analyze driver priority distribution
    priority_distribution = {"priority_1": 0, "priority_2": 0, "priority_3": 0, "priority_4": 0}

    for driver in (drivers_unassigned_data or []):
        shift_type_id = int(driver.get('shift_type_id', 2))
        if shift_type_id in [1, 3]:
            priority_distribution["priority_1"] += 1
        else:
            priority_distribution["priority_2"] += 1

    if drivers_assigned_data:
        for driver in drivers_assigned_data:
            shift_type_id = int(driver.get('shift_type_id', 2))
            if shift_type_id in [1, 3]:
                priority_distribution["priority_3"] += 1
            else:
                priority_distribution["priority_4"] += 1

    return {
        "total_demand": total_users,
        "total_capacity": total_capacity,
        "drivers_unassigned": len(drivers_unassigned_data or []),
        "drivers_assigned": len(drivers_assigned_data) if drivers_assigned_data else 0,
...
def calculate_performance_metrics(routes, unassigned_users, unassigned_drivers):
    """Calculate detailed performance metrics with edge case handling"""
    ...
    for route in routes:
        if route["assigned_users"]:
            util = len(route["assigned_users"]) / max(route["vehicle_type"], 1)  # Prevent division by zero
    ...
    return {
        ...
        "utilization_metrics": {
            "average_utilization_percent": round(np.mean(utilizations) * 100, 2),
            "min_utilization_percent": round(np.min(utilizations) * 100, 2),
            "max_utilization_percent": round(np.max(utilizations) * 100, 2),
        ...
        "distance_metrics": {
            "average_distance_km": round(np.mean(route_distances), 2),
            "max_distance_km": round(np.max(route_distances), 2),
...
def geographical_analysis(routes):
    ...
    if not route_centers:
        return {
            "error": "No routes with assigned users",
            "route_distribution": {
                "number_of_routes": 0,
                "geographical_spread": {
                    "lat_range": [0, 0],
                    "lng_range": [0, 0],
                    "center_of_operations": [0, 0]
                }
            },
            "clustering_effectiveness": {
                "average_intra_cluster_distance_km": 0,
                "tightest_cluster_km": 0,
                "loosest_cluster_km": 0,
                "well_clustered_routes": 0
            }
        }
    ...
    if len(route_centers) == 1:
        lat_range = [float(centers_df['lat'].iloc[0]), float(centers_df['lat'].iloc[0])]
        lng_range = [float(centers_df['lng'].iloc[0]), float(centers_df['lng'].iloc[0])]
    else:
...
def analyze_clustering_effectiveness(routes):
    ...
    for route in routes:
        ...
        elif len(route["assigned_users"]) == 1:
            # Single user routes have perfect clustering (0 distance)
            cluster_tightness.append(0.0)

    if not cluster_tightness:
        return {
            "average_intra_cluster_distance_km": 0,
            "tightest_cluster_km": 0,
            "loosest_cluster_km": 0,
    ...
    return {
        "average_intra_cluster_distance_km": round(np.mean(cluster_tightness), 2),
        "tightest_cluster_km": round(np.min(cluster_tightness), 2),
        "loosest_cluster_km": round(np.max(cluster_tightness), 2),
        "well_clustered_routes": sum(1 for t in cluster_tightness if t < 3.0)  # Routes with avg distance < 3km
    }

def optimization_insights(routes, users_data, drivers_unassigned_data, drivers_assigned_data=None):
    ...
    # Resource optimization - combine both driver lists
    all_drivers = (drivers_unassigned_data or []).copy()
    if drivers_assigned_data:
        all_drivers.extend(drivers_assigned_data)

    total_capacity = sum(int(d.get('capacity', 0)) for d in all_drivers)
    total_users = len(users_data)
...
def print_analysis_summary(analysis_report):
    ...
    print(f"  driversUnassigned: {raw_data['total_drivers_unassigned']}")
    print(f"  driversAssigned: {raw_data['total_drivers_assigned']}")
...
if __name__ == "__main__":
    ...
    PARAMETER = 1  # Replace with your parameter value
    STRING_PARAM = ""  # Replace with your string parameter

    print("🔍 Starting comprehensive analysis...")
    print("This will fetch data, run assignment, and generate detailed analysis")
    print(f"Using source_id: {SOURCE_ID}, parameter: {PARAMETER}, string_param: {STRING_PARAM}")

    result = comprehensive_analysis(SOURCE_ID, PARAMETER, STRING_PARAM)
