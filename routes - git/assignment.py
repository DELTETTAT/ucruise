...
SWAP_IMPROVEMENT_THRESHOLD = cfg.get("swap_improvement_threshold_km", 0.2)
MAX_SWAP_ITERATIONS = cfg.get("max_swap_iterations", 5)
...
def calculate_bearings_and_features(user_df, office_lat=None, office_lon=None):
    """Calculate bearings and add geographical features"""
    if office_lat is None:
        office_lat = OFFICE_LAT
    if office_lon is None:
...
def improved_geographical_clustering(user_df):
    ...
    # Use DBSCAN for density-based clustering with config parameters
    coords_rad = np.radians(coords)
    eps_rad = DBSCAN_EPS_KM / 6371.0  # Convert to radians
...
def swap_optimization(drivers_and_routes):
    """
    Optimize routes by swapping users between routes with performance improvements
    """
    improved = True
    iterations = 0
            ...
            for j, route_b in enumerate(drivers_and_routes[i+1:], start=i+1):
                ...
                for user_a in route_a['assigned_users'][:]:  # Copy to avoid modification during iteration
                    for user_b in route_b['assigned_users'][:]:
                        ...
                        # If swap improves total distance by threshold amount
                        if dist_a_swap + dist_b_swap + SWAP_IMPROVEMENT_THRESHOLD < dist_a_current + dist_b_current:
                            # Perform swap
...
def reassign_underutilized_users_to_nearby_routes(drivers_and_routes, unassigned_drivers):
    ...
    # STEP 0: Merge underfilled nearby routes - create safe copy to avoid iteration issues
    merged_routes = []
    routes_used = set()
    ...
    for i, route_a in enumerate(drivers_and_routes):
        ...
        merged = False
        for j, route_b in enumerate(drivers_and_routes[i+1:], start=i+1):
            if j in routes_used or route_b['utilization'] >= 1.0:
                continue
    ...
    # STEP 1: Fallback reassignment for low utilization, few users
    ...
    if FALLBACK_MIN_USERS <= len(combined_users) <= FALLBACK_MAX_USERS:
        ...
        if fallback_driver:
            # Only mark routes for removal if we successfully found a fallback driver
...
def run_assignment(source_id: str):
    ...
    try:
        ...
        print(f"🗂️ Geographical clustering complete - {clusters_found} clusters found")
