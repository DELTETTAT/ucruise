...
class RouteAssignmentLogger:
    def __init__(self, log_dir="logs"):
        ...
        # Create separate file for user/driver tracking
        self.tracking_file = os.path.join(log_dir, f"user_driver_tracking_{self.session_timestamp}.log")
    ...
    def log_user_unassigned(self, user_id, reason, attempted_drivers):
        ...
        with open(self.tracking_file, 'a') as f:
            f.write(f"UNASSIGNED_USER | {user_id} | {reason} | {datetime.now()}\n")
    ...
    def log_driver_unused(self, driver_id, reason, capacity, location):
        ...
        # Write to tracking file
        with open(self.tracking_file, 'a') as f:
            f.write(f"UNUSED_DRIVER | {driver_id} | {reason} | {capacity} | {location} | {datetime.now()}\n")
