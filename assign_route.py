...
class RoadCorridorAnalyzer:
    ...
    def _creates_zigzag_pattern(self, route_users, new_user_pos, driver_pos, office_pos):
        """Check if adding a new user would create a zigzag pattern (e.g., east then south)"""
        if not route_users:
            return False

        # Calculate bearings for the route segments
        current_pos = driver_pos
        bearings = []

        # Add bearings for existing route
        for user in route_users:
            user_pos = (user['lat'], user['lng'])
            bearing = calculate_bearing(current_pos[0], current_pos[1], user_pos[0], user_pos[1])
            bearings.append(bearing)
            current_pos = user_pos

        # Add bearing for new user
        new_bearing = calculate_bearing(current_pos[0], current_pos[1], new_user_pos[0], new_user_pos[1])
        bearings.append(new_bearing)

        # Check for significant direction changes (zigzag pattern)
        for i in range(len(bearings) - 1):
            bearing_change = abs(normalize_bearing_difference(bearings[i+1] - bearings[i]))

            # If there's a significant direction change (>60 degrees), it's likely a zigzag
    ...
    def assign_users_to_routes(self, users, drivers, office_pos, routes, assigned_user_ids, used_driver_ids, mode):
        ...
        logger.info(f"🔄 Starting assign_users_to_routes with {len(users)} users, {len(routes)} existing routes, mode: {mode}")
        ...
        logger.info(f"📋 Processing {len(sorted_users)} users sorted by office distance")

        for user in sorted_users:
            if str(user['id']) in assigned_user_ids:
                logger.debug(f"⏭️ Skipping user {user['id']} - already assigned")
                continue

            logger.info(f"👤 Evaluating user {user['id']} for assignment")
            logger.debug(f"   📍 User coordinates: ({float(user['latitude']):.6f}, {float(user['longitude']):.6f})")
            ...
            for route in routes:
                ...
                # STRICT DIRECTIONAL CONSISTENCY CHECK
                directional_consistency_passed = True
                if route_users:
                    # Calculate main route direction (driver to office)
                    main_bearing = calculate_bearing(driver_pos[0], driver_pos[1], office_pos[0], office_pos[1])

                    # Calculate user bearing from driver
                    user_bearing = calculate_bearing(driver_pos[0], driver_pos[1], user_pos[0], user_pos[1])

                    # Check bearing difference - much stricter for directional consistency
                    bearing_diff = abs(normalize_bearing_difference(user_bearing - main_bearing))
                    max_bearing_deviation = 30 if mode == "efficiency" else 45  # Stricter for efficiency mode

                    if bearing_diff > max_bearing_deviation:
                        logger.debug(f"   ❌ Route {route['driver_id']} rejected: Directional inconsistency ({bearing_diff:.1f}° > {max_bearing_deviation}°)")
                        directional_consistency_passed = False
                        continue

                    # Check for zigzag patterns - ensure user doesn't create east-then-south pattern
                    if self._creates_zigzag_pattern(route_users, user_pos, driver_pos, office_pos):
                        logger.debug(f"   ❌ Route {route['driver_id']} rejected: Would create zigzag pattern")
                        directional_consistency_passed = False
                        continue

                    logger.debug(f"   ✅ Directional consistency passed: {bearing_diff:.1f}° deviation within {max_bearing_deviation}°")
                ...
                if self.road_network:
                    ...
                    logger.debug(f"   🎯 Combined score: {combined_score:.3f} (threshold: 0.5)")

                    # Accept if combined score is reasonable - raised threshold for better quality
                    if combined_score < 0.5:  # Raised from 0.4
                        logger.debug(f"   ❌ Route {route['driver_id']} rejected: Combined score {combined_score:.3f} < 0.5")
                        continue
                    else:
                ...
                if route_type == "straight":
                    test_route['assigned_users'].sort(key=lambda u: u['office_distance'])
                    ...
                    optimized_sequence = self.road_network.get_optimal_pickup_sequence(
                        driver_pos,
                        [(u['lat'], u['lng']) for u in test_route['assigned_users']],
                        office_pos
                    ) if self.road_network else list(range(len(test_route['assigned_users'])))
            ...
            if best_route_for_user:
                logger.info(f"🎯 ROUTE ASSIGNMENT DECISION: User {user['id']} -> Route {best_route_for_user['driver_id']}")
                logger.info(f"   📊 Assignment Score: {best_route_score:.3f}")
                logger.info(f"   ⏰ Assignment Time: {time.strftime('%H:%M:%S')}")
                logger.info(f"   🎯 Assignment Reason: Best scoring route among {len(routes)} candidates")
                ...
                # Log detailed assignment reason with comprehensive context
                assignment_details = {
                    ...
                    'route_capacity_before': f"{len(best_route_for_user['assigned_users'])-1}/{best_route_for_user['vehicle_type']}",
                    'route_capacity_after': f"{len(best_route_for_user['assigned_users'])}/{best_route_for_user['vehicle_type']}",
                    'pickup_position': len(best_route_for_user['assigned_users']),
                ...
                # Additional detailed logging for tracking
                logger.info(f"   📝 ASSIGNMENT DETAILS:")
                logger.info(f"      🎯 User {user['id']} added to route {best_route_for_user['driver_id']} as pickup #{len(best_route_for_user['assigned_users'])}")
                logger.info(f"      📏 Distance from driver: {assignment_details['distance_to_driver']:.3f}km")
                logger.info(f"      🏢 Distance to office: {assignment_details['office_distance']:.3f}km")
                logger.info(f"      🚗 Route utilization: {assignment_details['route_capacity_after']}")
                logger.info(f"      📊 Assignment score: {assignment_details['route_score']:.3f}")
                logger.info(f"      🛣️ Route coherence: {assignment_details['coherence_score']:.3f}")

                logger.info(f"   ✅ User {user['id']} successfully assigned to route {best_route_for_user['driver_id']}")
            else:
                user_pos = (float(user['latitude']), float(user['longitude']))
                logger.warning(f"❌ ASSIGNMENT FAILED: No suitable route found for user {user['id']}")
                logger.warning(f"   📍 User position: ({user_pos[0]:.6f}, {user_pos[1]:.6f})")
                logger.warning(f"   📊 Evaluated {len(routes)} existing routes, none suitable")
                logger.warning(f"   ⏰ Failed at: {time.strftime('%H:%M:%S')}")

                # Log why each route was rejected for this user
                for i, route in enumerate(routes, 1):
                    driver_pos = (route['latitude'], route['longitude'])
                    distance = haversine_distance(driver_pos[0], driver_pos[1], user_pos[0], user_pos[1])
                    capacity_used = len(route['assigned_users'])
                    capacity_total = route['vehicle_type']

                    rejection_reason = []
                    if capacity_used >= capacity_total:
                        rejection_reason.append("at_capacity")
                    if distance > 15.0:
                        rejection_reason.append("too_far")

                    logger.warning(f"      Route {i} (Driver {route['driver_id']}): {distance:.3f}km, {capacity_used}/{capacity_total} capacity - Rejected: {', '.join(rejection_reason) if rejection_reason else 'scoring'}")

        logger.info(f"✅ Completed assign_users_to_routes: {len(assigned_user_ids)} total users assigned")
    ...
    def calculate_route_score(self, route, driver_pos, office_pos, optimized_sequence):
        ...
        if optimized_sequence:
            ...
            if direct_dist > 0:
                ...
                detour_penalty = detour_ratio * 4  # Stronger penalty for detours
...
def assign_routes_road_aware(data):
    ...
    for corridor_route in corridor_routes:
        ...
        while corridor_users and corridor_iterations < max_corridor_iterations:
            ...
            logger.info(f"    🚗 ROUTE CREATION: Creating new route for driver {driver_id}")
            logger.info(f"         📍 Driver position: ({driver['latitude']:.6f}, {driver['longitude']:.6f})")
            logger.info(f"         🎯 Driver capacity: {driver['capacity']}")
            logger.info(f"         🛣️ Corridor: {corridor_route['direction']} (bearing: {corridor_route['bearing']:.1f}°)")
            logger.info(f"         👥 Available users in corridor: {len(corridor_users)}")
