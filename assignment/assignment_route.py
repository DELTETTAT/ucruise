...
def load_env_and_fetch_data(source_id: str,
    ...
    # Ensure source_id is properly formatted and not a JSON string
    if isinstance(source_id, str) and source_id.startswith('{'):
        # If source_id contains JSON data, extract the actual source_id or use a default
        print(f"⚠️ WARNING: source_id appears to contain JSON data, using default source_id")
        source_id = "UC_unify_dev"  # Use a safe default
    
    # URL encode the string parameter to handle special characters
    import urllib.parse
...
def handle_remaining_users_improved(unassigned_users_df, driver_df, routes, office_lat, office_lon):
    """Handle remaining unassigned users with create-new-routes fallback"""
    if unassigned_users_df.empty:
        return []
...
def run_assignment(source_id: str, parameter: int = 1, string_param: str = ""):
    """
    Route-optimized assignment function with emphasis on straight, efficient routes
    """
    start_time = time.time()

    try:
        print("🚀 Starting ROUTE EFFICIENCY assignment...")
        ...
        # Build unassigned drivers
        assigned_driver_ids = set()
        for route in routes:
            assigned_driver_ids.add(route['driver_id'])
        ...
        print(f"✅ ROUTE EFFICIENCY assignment complete in {execution_time:.2f}s")
        ...
        return {
            ...
            "assignment_strategy": "route_efficiency",
            "algorithm_used": "sophisticated_route_optimization",
            "parameter": parameter,
