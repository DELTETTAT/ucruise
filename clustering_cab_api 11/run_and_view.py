...
def display_detailed_analytics(result):
    ...
    for i, route in enumerate(routes):
        ...
        shift_type_display = "N/A"
        
        # Check if this driver was from unassigned or assigned list
        for d in unassigned_drivers:
            if d.get("driver_id") == route["driver_id"] or str(d.get("id", "")) == route["driver_id"]:
                shift_type_id = d.get('shift_type_id', 'N/A')
                if shift_type_id in [1, 3]:
                    driver_source = "driversUnassigned(P1)"
                else:
                    driver_source = "driversUnassigned(P2)"
                shift_type_display = f"ST:{shift_type_id}"
                break
        
        # If not found in unassigned, check if it was from driversAssigned
        if driver_source == "Unknown":
            # This driver was likely from driversAssigned
            driver_source = "driversAssigned(P3/4)"
