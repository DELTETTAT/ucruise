...
def validate_input_data(data):
    ...
    if "users" not in data:
        raise ValueError("Data must contain 'users' key")

    users = data.get("users", [])

    if not users:
        raise ValueError("No users provided")
    if not drivers_unassigned and not drivers_assigned:
        raise ValueError(
            "No drivers provided (neither driversUnassigned nor driversAssigned)"
        )
    ...
    for i, user in enumerate(users):
        ...
        try:
            ...
            if not (-90 <= lat <= 90) or not (-180 <= lon <= 180):
                raise ValueError(
                    f"User {i} has invalid coordinates: {lat}, {lon}")
        except (ValueError, TypeError):
            raise ValueError(f"User {i} has invalid coordinate values")

    # Validate driver data structure for both lists
    required_driver_fields = ['id', 'capacity', 'latitude', 'longitude']
    all_drivers = drivers_unassigned + drivers_assigned

    for i, driver in enumerate(all_drivers):
        driver_type = "driversUnassigned" if i < len(
            drivers_unassigned) else "driversAssigned"
        for field in required_driver_fields:
            if field not in driver:
                raise ValueError(
                    f"{driver_type} driver {i} missing required field: {field}"
                )
        ...
        try:
            ...
            if capacity <= 0:
                raise ValueError(
                    f"{driver_type} driver {i} has invalid capacity: {capacity}"
                )
        except (ValueError, TypeError):
            raise ValueError(
                f"{driver_type} driver {i} has invalid capacity value")
        ...
        try:
            ...
            if not (-90 <= lat <= 90) or not (-180 <= lon <= 180):
                raise ValueError(
                    f"{driver_type} driver {i} has invalid coordinates: {lat}, {lon}"
                )
        except (ValueError, TypeError):
            raise ValueError(
                f"{driver_type} driver {i} has invalid coordinate values")
...
def enhanced_geographical_clustering(user_df, office_lat, office_lon):
    ...
    print(
        f"🗂️ Clustering complete: {user_df['geo_cluster'].nunique()} clusters using {clustering_method}"
    )
...
def optimize_driver_assignment(user_df, driver_df):
    ...
    for priority in [1, 2, 3, 4]:
        count = len(
            [d for d in available_drivers if d['priority'] == priority])
        priority_desc = {
        ...
        print(
            f"   Priority {priority}: {count} drivers - {priority_desc[priority]}"
        )
    ...
    for cluster_id, cluster_users in user_df.groupby('geo_cluster'):
        ...
        if total_users <= max_capacity:
            ...
            sub_clusters = min(max(1, round(total_users / avg_capacity)),
                               len(available_drivers), total_users)
        ...
        for sub_id, sub_users in unassigned_in_cluster.groupby('sub_cluster'):
            ...
            # Find the best driver by strictly prioritizing by priority level first
            for driver in available_drivers:
                ...
                # STRICT PRIORITY: Always choose lower priority number first, regardless of cost
                if (driver['priority'] < best_priority or 
                    (driver['priority'] == best_priority and (best_driver is None or cost < min_cost))):
                    min_cost = cost
                    best_driver = driver
    ...
    for i, route_a in enumerate(drivers_and_routes):
        ...
        for j, route_b in enumerate(drivers_and_routes):
            ...
            if dist <= MERGE_DISTANCE_KM and total_users <= max_capacity:
                ...
                route_b['utilization'] = len(
                    route_b['assigned_users']) / route_b['vehicle_type']
                route_b['stops'] = [[u['lat'], u['lng']]
                                    for u in route_b['assigned_users']]
                used_indices.add(i)
                merged = True
                break
        ...
        for j, route_b in enumerate(drivers_and_routes[i + 1:], start=i + 1):
            ...
            if len(combined_users) <= max(route_a['vehicle_type'],
                ...
                if dist <= MERGE_DISTANCE_KM and bearing_diff <= MAX_BEARING_DIFFERENCE:
                    ...
                    route_a['vehicle_type'] = max(route_a['vehicle_type'],
                                                  route_b['vehicle_type'])
                    route_a['utilization'] = len(
                        combined_users) / route_a['vehicle_type']
                    routes_used.add(j)
                    merged = True
                    break
    ...
    for i, source_route in enumerate(drivers_and_routes):
        ...
        for user in source_route['assigned_users']:
            ...
            for j, target_route in enumerate(drivers_and_routes):
                if j == i or len(target_route['assigned_users']
